    ...
    while let Some(event) = rx.recv().await {
        ...
        for (target_node_id, peer_addr) in &targets {
            ...
            for (chunk_index, _size, _sha256, backend_path, dedup_sha) in &chunks {
                ...
                let client = PeerClient::new(&state.config.peer.secret);
                match client.push_chunk_full(
                    &peer_addr, &event.volume_id, event.file_id, *chunk_index, data,
                    &event.rel_path, &state.node_id,
                ).await {
                    Ok(_) => {
                        ...
                        if let Ok(chunk_id) = crate::services::chunk::ChunkService::get_chunk_id(
                            ...
                            log_err!(crate::services::chunk::ChunkService::track_remote_replica(
                                &db, chunk_id, target_node_id,
                            ), "push_replicator: track remote replica");
                        }
                        drop(db);
