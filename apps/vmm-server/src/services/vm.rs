...
impl VmService {
    pub fn list(db: &Connection) -> Result<Vec<VmRecord>, String> {
        let mut stmt = db.prepare(
            "SELECT id, name, config_json, owner_id, created_at FROM vms ORDER BY name"
        ).map_err(|e| e.to_string())?;
        let vms = stmt.query_map([], |row| {
            let config_json: String = row.get(2)?;
            Ok((row.get::<_,String>(0)?, row.get::<_,String>(1)?, config_json, row.get::<_,i64>(3)?, row.get::<_,String>(4)?))
        }).map_err(|e| e.to_string())?
        .filter_map(|r| r.ok())
        .map(|(id, name, config_json, owner_id, created_at)| {
            let config: VmConfig = serde_json::from_str(&config_json).unwrap_or_default();
            VmRecord { id, name, config, owner_id, created_at }
        }).collect();
        Ok(vms)
    }
